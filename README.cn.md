...
## 虽好奇但严谨：加速良作、有深入思考的小众作品推广，但严守安全和隐私底线。

我们乐意接收一些有趣的新软件，并且不唯 stars 和人气而论，我们不拒绝闭源和商业作品，也不拒绝 AI 生成的作品（但我们拒绝那些没有人类思考，没有安全考虑，没有长期维护计划的软件）。

但是，我们不打算成为营销页。尤其下列涉及**用户安全和隐私**的情况直接拒（已收录的也会黄标，甚至下架）：

- 有**恶意行为**的工具
- **收集用户隐私**但不明确、显著披露的工具
- **隐瞒自身行为**的工具 —— 未文档化的网络调用、隐藏的后台进程、混淆的载荷

用 `x install` 装什么软件到机器上，最终是用户的选择；但在索引这一侧，我们尽量帮用户把一道关。
